  ...
  <div class="index-view">
    <header class="index-header mb-6 flex items-end justify-between">
      <div class="header-info">
        <h1 class="text-3xl font-bold font-serif text-[#2a0002]">Variedades</h1>
        <p class="text-gray-600 mt-2">Gestiona las cepas y variedades de uva.</p>
      </div>
      <div class="header-actions">
        <router-link :to="{ name: 'admin.variedades.create' }" class="btn-primary">
      ...
      <table v-else class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-gray-50 border-b border-gray-200 text-gray-600 text-xs uppercase tracking-wider">
            ...
            <th class="p-4 font-medium">Nombre de la Cepa / Variedad</th>
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          >
            <td class="p-4 text-sm text-gray-500">#{{ item.id_variedad }}</td>
...
async function fetchItems() {
  loading.value = true
  try {
    const { data } = await api.get('/admin/variedades')
...
async function confirmDelete(item) {
  ...
  try {
    await api.delete(`/admin/variedades/${item.id_variedad}`)
    notif.show('Variedad eliminada con éxito.', 'success')
    fetchItems()
  } catch (err) {
    const msg = err.response?.data?.message || 'Error al eliminar la variedad. Puede estar en uso.'
